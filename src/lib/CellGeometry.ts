...
export class CellGeometry {
	...
	constructor(ghostMode: boolean) {
		...
		this.drawMesh = this.init_intanced_mesh(
			new THREE.PlaneGeometry(),
			new DrawableCellMaterial(),
		);
		this.pickMesh = this.init_intanced_mesh(
			new THREE.PlaneGeometry(),
			new PickableCellMaterial(),
		);
	}

	private init_intanced_mesh(
	...
	private getCellColor(cell: Cell, selected: boolean): THREE.Color {
		if (this.ghostMode) return new THREE.Color(0.5, 0.5, 0.5);
	...
	private update_instanced_mesh(
		mesh: InstancedMesh2,
		cells: Cell[],
		selectedCells: Set<number>,
		architecture: CellArchitecture,
	) {
		const init_cell_instance = (
			...
			.map((cell) => {
				...
				return {
					...
					color: this.getCellColor(
						cell,
						selectedCells.contains(cells.indexOf(cell)),
					),
				};
			});
	}
