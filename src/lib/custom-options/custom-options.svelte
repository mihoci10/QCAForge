<script lang="ts">
	...
	let { optionList, optionValues = $bindable() }: Props = $props();
	...
	{:else if option.type === "input"}
		<div class="flex flex-col gap-1.5">
			{#if option.descriptor.type === "slider"}
				...
				<div class="flex items-center gap-2">
					<Slider
						type="single"
						id={option.id}
						value={(optionValues.get(option.id) as number) ||
							option.descriptor.default}
					...
					<span class="min-w-16 text-right">
						{(
							(optionValues.get(option.id) as number) ||
							option.descriptor.default
						).toFixed(1)}
			...
			{:else if option.descriptor.type === "boolean"}
				...
				<Switch
					id={option.id}
					checked={(optionValues.get(option.id) as boolean) ||
						option.descriptor.default}
