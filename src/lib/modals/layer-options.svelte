<script lang="ts">
	...
	$effect(() => {
		selected_arch_id = layer?.cell_architecture_id;
	});
	let selected_arch_display = $derived(
		selected_arch_id
			? cell_architectures.get(selected_arch_id)?.name
	...
	function add_new_architecture() {
		let new_cell_architecture = createCellArchitecture(
			"New architecture",
			20,
			5,
			4,
		...
		while (
			cell_architectures
				.values()
				.some((arch) => arch.name == new_cell_architecture.name)
		) {
	...
	function remove_selected_architecture() {
		if (!selected_arch_id) return;

		const index = cell_architectures
			.keys()
			.toArray()
			.indexOf(selected_arch_id);
		if (index < 0) return;
		...
		if (new_index < 0) selected_arch_id = undefined;
		else selected_arch_id = cell_architectures.keys().toArray()[new_index];
	}
	...
	function applyCallbackInternal(data: any) {
		...
		layer.cells.forEach((cell) => {
			let polarization = getPolarization(
			...
			while (polarization.length < cell_architecture.dot_count / 4)
				polarization.push(0);
			cell.dot_probability_distribution =
				generateDotDistribution(polarization);
		});
...
<BaseModal
	bind:open={isOpen}
	type="confirm"
	applyCallback={applyCallbackInternal}
>
	{#snippet title()}
		{layer.name} settings
	{/snippet}
		...
		<div class="flex flex-col gap-1.5">
			...
			<div class="flex gap-2">
				...
				<Button
					variant="secondary"
					size="icon"
					onclick={() => remove_selected_architecture()}
				>
				...
				<Button
					variant="outline"
					size="icon"
					onclick={() => openCellArchitectureOptions()}
				>
