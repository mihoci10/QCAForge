...
export function createCellArchitecture(
	name: string,
	side_length: number,
	dot_diameter: number,
	dot_count: number,
...
export function serializeCellArchitecture(
...
export function getDotRadius(cell_architecture: CellArchitecture): number {
	return Math.sqrt(
		Math.pow(cell_architecture.dot_positions[0][0], 2) +
			Math.pow(cell_architecture.dot_positions[0][1], 2),
	);
}
...
const DEFAULT_CELL_ARCHITECTURES: CellArchitecture[] = [
	...
	createCellArchitecture(
		...
		10,
		8,
		(20.0 * 2.0) / 3.0 / (2.0 * Math.sin(Math.PI / 8.0)),
		"tri_state_60",
	),
	createCellArchitecture(
		"Tri state 72",
...
> {
	...
	for (let i = 0; i < DEFAULT_CELL_ARCHITECTURES.length; i++) {
		cell_architectures.set(
			DEFAULT_CELL_ARCHITECTURES[i].id,
			DEFAULT_CELL_ARCHITECTURES[i],
		);
	}
