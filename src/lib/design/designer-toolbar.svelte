<script lang="ts">
    ...
    let { inputModeIdx = $bindable(), snapEnabled = $bindable(), snapDivider = $bindable() }: Props = $props();
...
<div class="absolute {SNAP_BAR_POS} z-10 bg-background p-1 rounded-md">
    <div class='flex-row gap-1'>
        <div class='flex flex-row'>
            <Button variant='ghost' size='icon' class='data-[state=on]:bg-accent border-r-0'
            data-state={snapEnabled ? "on" : "off"}
            onclick={() => snapEnabled = !snapEnabled}>
                <Icon width={24} icon='material-symbols:grid-on-outline'/>
            </Button>
            <Input class='w-12 p-1 border-l-0' bind:value={snapDivider} type='number' min={1} max={500} step={1}/>
        </div>
    </div>
</div>