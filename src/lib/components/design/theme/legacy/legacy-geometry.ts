...
import {
	LegacyDrawableCellMaterial,
	LegacyPickableCellMaterial,
} from "./legacy-material";

export class LegacyCellGeometry implements ICellGeometry {
	...
	constructor(props: CellGeometryProps) {
		...
		this.drawMesh = this.initInstancedMesh(
			...
			new LegacyDrawableCellMaterial(),
		);
		this.pickMesh = this.initInstancedMesh(
			new THREE.PlaneGeometry(),
			new LegacyPickableCellMaterial(),
		);
	}
