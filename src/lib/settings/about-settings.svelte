<script lang="ts">
	...
	function sendFeedback() {
		openExternal(
			"mailto:miha.krajnc8@gmail.com?subject=QCA Forge Feedback",
		);
	}
</script>
	...
	<div class="space-y-6">
		<div class="text-center space-y-2">
			...
			<p class="text-muted-foreground">
				Quantum-dot Cellular Automata Design & Simulation
			</p>
			<div class="flex justify-center gap-2">
				...
				{#if appInfo.buildInfo.git_branch !== "main"}
					<Badge variant="secondary"
						>{appInfo.buildInfo.git_branch}</Badge
					>
				{/if}
			</div>
		</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div class="space-y-2">
					...
					<p class="text-sm text-muted-foreground font-mono">
						{appInfo.buildInfo.version}
					</p>
				</div>
				<div class="space-y-2">
					<Label>QCACore Version</Label>
					<p class="text-sm text-muted-foreground font-mono">
						{appInfo.buildInfo.qca_core_version}
					</p>
				</div>
				<div class="space-y-2">
					<Label>Build Number</Label>
					<p class="text-sm text-muted-foreground font-mono">
						{appInfo.buildInfo.git_sha}
					</p>
				</div>
				<div class="space-y-2">
					<Label>Build Date</Label>
					<p class="text-sm text-muted-foreground">
						{appInfo.buildInfo.timestamp}
					</p>
				</div>
			</div>
		</div>
