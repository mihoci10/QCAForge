<script lang="ts">
	...
	function selectedClockModeChanged(newClockMode: string) {
		...
		if (isNaN(+selectedClockMode)) return;

		const clock_phase_shift = parseInt(selectedClockMode);

		selectedCells.forEach((id) => {
			layers[id.layer].cells[id.cell].clock_phase_shift =
				clock_phase_shift;
		});
		propertyChangedCallback();
	}
	...
	function selectedCellTypeChanged(newCellType: string) {
		...
		if (isNaN(+selectedCellType)) return;
	...
	function selectedCellRotationChanged(newCellRotation: string) {
		...
		if (isNaN(+selectedCellRotation)) return;
	...
	function polarizationInputChanged() {
		...
		selectedCells.forEach((id) => {
			layers[id.layer].cells[id.cell].dot_probability_distribution =
				generateDotDistribution(polarizationInput);
		});
		propertyChangedCallback();
	}
	...
	function positionInputChanged(ind: number) {
		selectedCells.forEach((id) => {
			if (isNaN(positionInput[ind])) return;
	...
	export function selectedCellsUpdated() {
		...
		selectedCells.forEach((id) => {
			...
			for (let i = 0; i < 2; i++) cellPositions[i].add(cell.position[i]);
		});

		if (clockModes.size() > 1) selectedClockMode = "multiple";
		else if (clockModes.size() == 1)
			selectedClockMode = clockModes.toArray()[0].toString();

		if (cellTypes.size() > 1) selectedCellType = "multiple";
		else if (cellTypes.size() == 1)
			selectedCellType = cellTypes.toArray()[0].toString();

		if (cellRotation.size() > 1) selectedCellRotation = "multiple";
		else if (cellRotation.size() == 1)
			selectedCellRotation = cellRotation.toArray()[0].toString();

		if (cellArchitecture.size() == 1) {
			let equalCheck = polarizations.reduce(
				(acc, v) => acc && v.size() == 1,
				true,
			);
			if (equalCheck)
				polarizationInput = polarizations.map((v) => v.toArray()[0]);
			else polarizationInput = new Array(polarizations.length).fill(NaN);
		} else if (cellArchitecture.size() > 1) polarizationInput = [NaN];
