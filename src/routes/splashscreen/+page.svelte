<script lang="ts">
	...
	onMount(() => {
		...
		const unlistenStartup = appWebview.listen(
			EVENT_STARTUP_PROGRESS,
			(event) => {
...
<div
	class="flex flex-col items-center justify-center h-screen w-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden"
>
	...
	<!-- Main content -->
	<div
		class="flex flex-col items-center justify-center relative z-10 transition-all duration-1000 ease-out transform {mounted
	...
	>
		...
		<div class="relative mb-8">
			<div
				class="w-24 h-24 bg-gradient-to-br from-blue-400 via-purple-500 to-pink-500 rounded-2xl shadow-2xl shadow-purple-500/25 flex items-center justify-center transform hover:scale-105 transition-transform duration-300"
			>
				<div
			...
			<!-- Glow effect -->
			<div
				class="absolute inset-0 bg-gradient-to-br from-blue-400 via-purple-500 to-pink-500 rounded-2xl blur-xl opacity-30 animate-pulse"
			></div>
		</div>

		<!-- App name -->
		<h1
			class="text-4xl font-bold bg-gradient-to-r from-white via-blue-100 to-purple-200 bg-clip-text text-transparent mb-2 tracking-wide"
		>
			QCAForge
		</h1>
		...
		<div class="w-96 max-w-sm">
			...
			<div class="mb-4 text-center">
				<p
					class="text-gray-300 text-sm font-medium transition-all duration-300 min-h-[1.25rem]"
				>
					{status}
				</p>
			</div>
		...
		<div class="mt-2">
			<div
				class="w-6 h-6 border-2 border-gray-600 border-t-purple-500 rounded-full animate-spin"
			></div>
		</div>
	</div>
</div>
