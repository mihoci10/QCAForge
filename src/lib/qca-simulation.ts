...
import {
	deserializeQCADesign,
	deserializeQCADesignFile,
	type CommonSimulationModelSettings,
	type QCADesign,
} from "./qca-design";
...
export class QCASimulation {
	...
	public async getInputData(input: PanelInput): Promise<Float64Array[]> {
		switch (input.type) {
			case InputType.CELL:
				const cellIndex = input.index as CellIndex;
				const archId =
					this._design.layers[cellIndex.layer].cell_architecture_id;
				const arch = this._design.cell_architectures.get(archId)!;
				const polarizationN = arch.dot_count / 4;
				return Promise.all(
					Array.from({ length: polarizationN }, (_, i) =>
						this.getCellData(cellIndex, i),
					),
				);
			case InputType.SIGNAL:
				const signalIndex = input.index as SignalIndex;
				switch (signalIndex.type) {
					...
					case SignalType.CELL:
						const cellIndex =
							this._metadata.stored_cells[signalIndex.index];
	...
	private getSignals(): Signal[] {
		...
		for (let i = 0; i < this._metadata.stored_cells.length; i++) {
			...
			for (let j = 0; j < polarization_n; j++) {
				let cellName =
					this._design.layers[layer].cells[cellIndex.cell].label;
	...
	public getSignal(signalIndex: SignalIndex): Signal {
		...
		for (const signal of signals) {
			if (
				signal.index.type === signalIndex.type &&
				signal.index.index === signalIndex.index
			) {
				if (
					signalIndex.subindex === undefined ||
					signal.index.subindex === signalIndex.subindex
				) {
					return signal;
				}
			}
	...
	public loadData(): Promise<void> {
		return new Promise((resolve, reject) => {
			const url =
				convertFileSrc("", "load-sim") + `?filename=${this._filename}`;
			fetch(url, {
				method: "GET",
			})
				.then((response) => {
					if (response.status === 200) {
						response
							...
							.then((buffer) => {
								...
								for (
									let i = 0;
									i < this._clockData.length;
									i++
								) {
									this._clockData[i] = new Float64Array(
										num_samples,
									);
								...
								let buf_offset =
									this._clockData.length * num_samples;
								for (const cell of this._metadata
									.stored_cells) {
									const layer = cell.layer;
									const arch_id =
										this._design.layers[layer]
											.cell_architecture_id;
									const arch =
										this._design.cell_architectures.get(
											arch_id,
										)!;
									const polarization_n = arch.dot_count / 4;
		...
		invoke("load_simulation_file", { filename: filename })
			.then((result: unknown) => {
				...
				const metadata = deserializeMetadata(
					JSON.stringify(resultPair[1]),
				);
				const design = deserializeQCADesign(
					JSON.stringify(resultPair[0]),
				);
